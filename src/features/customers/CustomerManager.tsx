            ...
            <div className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
                ...
                <div className="md:hidden divide-y divide-gray-200">
                    {filteredCustomers.map((customer) => (
                        <div key={customer.id} className="p-4 hover:bg-gray-50 active:bg-gray-100 transition-colors">
                            ...
                            <div className="space-y-1.5">
                                {customer.email && (
                                    <div className="flex items-center gap-2 text-sm text-gray-600">
                                        <span className="w-12 text-[10px] uppercase text-gray-400 font-bold">Email</span>
                                        <span className="truncate">{customer.email}</span>
                                    </div>
                                )}
                                {customer.phone && (
                                    <div className="flex items-center gap-2 text-sm text-gray-600">
                                        <span className="w-12 text-[10px] uppercase text-gray-400 font-bold">Tel</span>
                                        <span>{customer.phone}</span>
                                    </div>
                                )}
                                {customer.address && (
                                    <div className="mt-2 pt-2 border-t border-gray-50 flex flex-col gap-1">
                                        <span className="text-[10px] uppercase text-gray-400 font-bold">Dirección</span>
                                        <p className="text-sm text-gray-500 leading-tight">{customer.address}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {filteredCustomers.map((customer) => (
                                <tr key={customer.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4 font-medium text-gray-900 text-sm">{customer.name}</td>
                                    <td className="px-6 py-4 text-gray-600 text-sm">{customer.email}</td>
                                    <td className="px-6 py-4 text-gray-600 text-sm whitespace-nowrap">{customer.phone}</td>
                                    <td className="px-6 py-4 text-gray-600 text-sm max-w-xs truncate">{customer.address}</td>
